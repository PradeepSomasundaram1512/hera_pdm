...
\textit{Key findings.} On two public bearing datasets with contrasting degradation behaviour, (i)~calibration removes most unsafe decisions of point-threshold maintenance; (ii)~uncertainty-gated escalation pays off only when degradation is observable early; (iii)~the precise gating rule matters less than the interval width; and (iv)~snapshot-level guarantees do not yield timely bearing-level alarms, while bearing-level guarantees need larger fleets than public datasets provide.
...
\item Snapshot-level, bearing-level and sensitivity evidence, with bootstrap intervals over bearings, that turns findings (i)--(iv) into six deployment lessons.
\end{enumerate}
...
\begin{tabular}{p{2.35cm}p{2.3cm}p{3.3cm}}
\toprule
...
Alarm-timing guarantee needed & Conformal risk control with $n\ge(1{-}\varepsilon)/(\varepsilon{-}\pi)$ units & with $\approx$12 units: late \feLateCrcTen\%/\xjLateCrcTen\% but premature \fePremCrcTen\%/\xjPremCrcTen\% \\
