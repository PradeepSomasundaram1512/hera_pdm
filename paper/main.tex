...
\title{HERA-PdM: When Does Uncertainty-Gated Escalation Help Hierarchical Edge--Digital-Twin Predictive Maintenance? Evidence from Two Bearing Datasets}
...
\begin{abstract}
Hierarchical predictive-maintenance (PdM) systems escalate from edge devices to fog/cloud prognostics, usually on anomaly thresholds or uncalibrated confidence. We study HERA-PdM, in which cross-validated conformalized quantile regression (CQR-CV+) gives remaining-useful-life (RUL) intervals, the edge escalates only when its interval does not fix a unique maintenance action, and high-impact actions are SHAP-supported recommendations for human approval. On 17 FEMTO and 15 XJTU-SY run-to-failure bearings (bearing-grouped cross-validation, three seeds, tuned LSTM/GRU/TCN/CNN-LSTM baselines), CQR-CV+ raised coverage to \fePicpEdge--\fePicpCloud{} and \xjPicpCloud--\xjPicpEdge{} (nominal 0.90). Versus point estimates with fixed thresholds, calibrated decisions cut critical snapshots left without action from \feUnsafePoint\% to \feUnsafeHera\% (FEMTO) and \xjUnsafePoint\% to \xjUnsafeHera\% (XJTU-SY), at the cost of more false maintenance. On gradually degrading XJTU-SY bearings, escalating \xjEscHera\% of snapshots matched always-cloud safety with less false maintenance (\xjFmHera\% vs.\ \xjFmCloud\%) and payload; on abrupt FEMTO failures it offered no benefit. The decision-sufficiency gate performed no better than an interval-width gate, and all calibrated policies raised maintenance alarms prematurely for most bearings. Uncertainty-gated hierarchies thus pay off only when degradation is observable early.
\end{abstract}
...
\textit{Limitations.} Two bearing datasets (32 bearings); approximate exchangeability; a lightweight state-based DT; host-CPU latency and analytic communication; $R_{\max}$ sensitivity with one seed; no deployment and no user study of the explanation cards.

\section{Conclusion}
Conformal RUL intervals from cross-fitted calibration removed most unsafe decisions of point-threshold maintenance on two bearing datasets. Using them to gate edge-to-cloud escalation paid off only on gradually degrading bearings, where it matched always-cloud safety with less false maintenance and communication; on abruptly failing bearings it escalated almost everything. The specific decision-sufficiency rule was no better than a width gate, and per-snapshot calibration produced premature bearing-level alarms. Future hierarchical PdM should calibrate alarm timing rather than snapshot coverage and should decide escalation from observed degradation onset.
