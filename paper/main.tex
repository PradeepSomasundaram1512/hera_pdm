...
\title{HERA-PdM: Lessons from Deploying Uncertainty-Gated Edge AI for Predictive Maintenance on Two Bearing Datasets}
...
\begin{abstract}
Deploying AI for predictive maintenance (PdM) requires deciding where inference runs, how far predictions can be trusted, and when humans must act. Hierarchical systems usually escalate from edge to cloud on anomaly thresholds. We study HERA-PdM, in which cross-validated conformalized quantile regression (CQR-CV+) gives remaining-useful-life (RUL) intervals, the edge escalates only when its interval does not fix a unique maintenance action, and high-impact actions are SHAP-supported recommendations for human approval. On 17 FEMTO and 15 XJTU-SY run-to-failure bearings (bearing-grouped cross-validation, three seeds, tuned LSTM/GRU/TCN/CNN-LSTM baselines), CQR-CV+ raised coverage to \fePicpEdge--\fePicpCloud{} and \xjPicpCloud--\xjPicpEdge{} (nominal 0.90). Versus point estimates with fixed thresholds, calibrated decisions cut critical snapshots left without action from \feUnsafePoint\% to \feUnsafeHera\% (FEMTO) and \xjUnsafePoint\% to \xjUnsafeHera\% (XJTU-SY), at the cost of more false maintenance. On gradually degrading XJTU-SY bearings, escalating \xjEscHera\% of snapshots matched always-cloud safety with less false maintenance (\xjFmHera\% vs.\ \xjFmCloud\%) and payload; on abrupt FEMTO failures it offered no benefit. The decision-sufficiency gate performed no better than an interval-width gate, all calibrated policies raised maintenance alarms prematurely for most bearings, and conformal risk control met a per-bearing lateness target only by alarming early. Uncertainty-gated hierarchies thus pay off only when degradation is observable early.
\end{abstract}
...
Predictive maintenance (PdM) estimates the remaining useful life (RUL) of machinery from condition data so that interventions are scheduled before failure~\cite{lei2018machinery}. Deep sequence models are the prevailing RUL estimators~\cite{zheng2017lstm,zhu2019mscnn}, but in a factory three further requirements arise. Vibration is sensed at the machine, so edge--cloud architectures keep routine inference local~\cite{ren2021cloudedge}; a point estimate must be accompanied by calibrated uncertainty~\cite{basora2025benchmark,javanmardi2023conformal}; and Industry~5.0 keeps the human in charge~\cite{xu2021industry5}, since decision-makers often do not adopt system-generated maintenance advice~\cite{vanoudenhoven2023pdm5}.

Recent systems combine these elements with agents and Digital Twins (DTs)~\cite{tao2019dt,saleh2026hama,mendonca2026human,coulibaly2026masdt}; our audit of 42 papers (Section~\ref{sec:related}) shows that this \emph{combination} is no longer new. What remains open is the \emph{escalation rule}: hierarchical PdM systems escalate on anomaly thresholds~\cite{saleh2026hama} or uncalibrated confidence~\cite{yao2026ecdistill}, while calibrated conformal deferral has been studied for classification and numerical cascades~\cite{huang2025cab,hou2025online}. We ask \emph{when} conformally calibrated RUL intervals on the edge are a useful escalation signal, and at what cost in accuracy, safety, alarm timing, communication and latency.

Contributions:
\begin{enumerate}
\item \textbf{HERA-PdM}, a reproducible four-layer framework with a \emph{decision-sufficiency} gate that escalates only when the edge RUL interval straddles a maintenance-decision boundary, bounding locally resolved unsafe decisions by the edge miscoverage level, and CQR-CV+ calibration on all non-test assets.
\item A two-dataset, bearing-grouped benchmark (FEMTO with abrupt, XJTU-SY with gradual degradation) of ten tuned quantile models with and without DT context, showing that CQR-CV+ restores nominal coverage and that larger fog/cloud models help only on the gradual dataset.
\item Snapshot-level, per-bearing and sensitivity evidence that calibration removes most unsafe point-threshold decisions, that uncertainty-gated escalation improves on always-cloud inference only under gradual degradation, and that the specific gate matters less than the interval width.
\end{enumerate}

\section{Related Work and Research Gap}\label{sec:related}
\textit{RUL prediction.} LSTM~\cite{zheng2017lstm}, multiscale CNN for bearings~\cite{zhu2019mscnn} and TCN~\cite{bai2018tcn} models are standard; reviews~\cite{lei2018machinery} treat health-stage division, i.e.\ detecting when degradation becomes observable, as a step preceding RUL prediction, which our two datasets~\cite{nectoux2012pronostia,wang2020xjtu} contrast sharply.
\textit{DTs and agents.} DTs provide a synchronized virtual asset state~\cite{tao2019dt}; agentic and human-centred DT architectures for PdM have appeared~\cite{mendonca2026human,coulibaly2026masdt}, and~\cite{coulibaly2026masdt} lists hierarchical DT orchestration with residual-life estimation and explainability as an open question.
...
\textit{Uncertainty.} Bayesian, dropout and ensemble RUL uncertainty has been benchmarked in~\cite{basora2025benchmark}; split conformal and CQR give finite-sample coverage~\cite{romano2019cqr,angelopoulos2023gentle}, CV+ extends it to cross-fitting~\cite{barber2021jackknife}, and conformal RUL intervals exist for C-MAPSS~\cite{javanmardi2023conformal,moccardi2025robust} and bearings~\cite{wang2026conformal}. Probabilistic RUL also drives risk-aware maintenance policies~\cite{xu2025riskrl}, and confidence-based~\cite{jitkrittum2023cascade} and conformal~\cite{huang2025cab,hou2025online} deferral are the closest methodological precedents.

\textbf{Gap.} To the best of our review (Table~\ref{tab:related}), no study uses conformal RUL intervals computed on the edge as the escalation rule of a hierarchical PdM system, relates escalation to maintenance-decision boundaries, and evaluates it against the gates of existing systems on public run-to-failure data with different degradation regimes. The contribution is methodological and evaluative; the architecture is not new.
...
\textit{Bearing-level risk control.} We also calibrated the alarm offset directly for timing: an onset-gated two-stage alarm whose offset $\lambda$ was chosen by conformal risk control~\cite{angelopoulos2024crc} on the CV+ out-of-fold predictions of the non-test bearings, so that $\Pr(\text{late or missing alarm})\le\varepsilon$ per bearing. At $\varepsilon{=}0.1$ the held-out late rate stayed within the target (\feLateCrcTen\% FEMTO, \xjLateCrcTen\% XJTU-SY vs.\ \feLatePointHp\% and \xjLatePointHp\% for the point policy), but only by alarming early: \fePremCrcTen\% and \xjPremCrcTen\% of alarms were premature and the onset gate escalated \feEscCrcTen\% and \xjEscCrcTen\% of snapshots. With about 12 calibration bearings, $\varepsilon{=}0.1$ tolerates no late calibration bearing, so the offset must cover the least predictable one; bearing-level guarantees need fleets with many more run-to-failure units.
...
\textit{Lessons for deployment.} (i)~\emph{Calibrate before routing}: calibrated intervals removed most unsafe point-threshold decisions on both datasets. (ii)~\emph{Hierarchy pays only if degradation is observable early}: escalation helped on XJTU-SY and not on FEMTO. (iii)~\emph{Small fleets strain calibration}: three calibration bearings under-covered, cross-fitting on all assets fixed pooled coverage, and per-asset coverage still varied. (iv)~\emph{The interval width matters more than the gate}: decision-sufficiency and width gates coincided. (v)~\emph{Snapshot guarantees are not alarm guarantees}: timely bearing-level alarms need larger fleets. (vi)~\emph{Wide intervals move work to humans}: approval load, not model accuracy, limits human-centric use. Limitations: 32 bearings, approximate exchangeability, a state-based DT, host-CPU latency, analytic communication, one-seed $R_{\max}$ sensitivity, and no deployment or user study.

\section{Conclusion}
Conformal RUL intervals from cross-fitted calibration removed most unsafe decisions of point-threshold maintenance on two bearing datasets. Using them to gate edge-to-cloud escalation paid off only on gradually degrading bearings, where it matched always-cloud safety with less false maintenance and communication; on abruptly failing bearings it escalated almost everything. The specific decision-sufficiency rule was no better than a width gate, and per-snapshot calibration produced premature bearing-level alarms. Calibrating alarm timing directly with conformal risk control met a bearing-level lateness target but, with about a dozen calibration bearings, only by alarming early. Useful timing guarantees will require larger run-to-failure fleets and features with longer warning lead times.
