...
* **Automated API search** (`docs/literature/lit_search.py`, raw hits in `docs/literature/search_raw/initial.json`):
...
* **Result:** 38 papers coded in the initial audit plus 4 added in the final re-search (§7) = 42 in `docs/literature/literature_matrix.csv` (preprints marked as such).
...
## 12. Version 4: evidence-calibrated reframing (2026-09-24)
* Title and narrative changed from a deployment/system claim to an evaluation question: *when* does uncertainty-gated edge-cloud PdM help? HERA-PdM is presented as an experimental framework; the width-gate equivalence, the FEMTO failure of hierarchy and the near-parity of calibrated edge-only inference on SCANIA are reported as central findings.
* Terminology: "Digital Twin" defined as a lightweight state-based representation (contrasted with Tao et al. 2019); "agents" = software modules only; human approval = implemented logic, not a validated human-in-the-loop effect; SHAP = attribution context, not validated explanation.
* New analyses (no new architecture): per-bearing coverage heterogeneity (median/min/count below 0.90), five-seed repeat of the key policy comparisons, runner-up architecture check (second-best inner-validation models), HERA-vs-width FM test (previously only unsafe was tested; FM differs on a few bearings by <=0.5 pp), Holm correction over five primary tests.
* A bug in the first combined robustness analysis (dataset constants cached across datasets in one process) produced invalid XJTU-SY numbers; detected because every policy showed 0 % unsafe / 100 % FM; fixed by one process per dataset (`analyze_all`). Three-seed reruns reproduce Table III exactly for both datasets.
* References extended from 15 to 25; all ten additions verified against Crossref DOI records, the JMLR page or the NeurIPS proceedings page, and cited only for what their abstracts state (docs/literature/reference_verification.csv).
* Removed from the paper for space: trajectory figure, detection/compute table, deployment-guide table, explanation card (their key numbers remain in the text).
